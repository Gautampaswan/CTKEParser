...
class emailFilter {
    ...
    constructor(page) {
        ...
        this.sourceOptionName = 'Subject'; //Need to change as per requirement
        ...
        this.operationOptionName = 'Ends With'; //Need to change as per requirement and compare with Source Option

        
        //Email Filter Record Name for Validation 
...
this.EPreturn = page.locator("[data-field-id='Recordctkemailparser_EmailParser_cField'] div.slds-grid").last();
    ...
    async newEmailFilter(email_Filter_Name, filter_Value, email_Sequence) {
        ...
        await expect(this.emailFilterNameValidation).toHaveText("Subject_Verification");

        //To Validate the Operation in Email Filter .
        await this.page.waitForTimeout(3000);
        await expect(this.operationValidation).toHaveText("Ends With");
	            
        //To Validate the Object Type.
        await this.page.waitForTimeout(3000);
        await expect(this.valueValidation).toHaveText("CTK Email Parser.");
