...
class fieldParser_2 {
    ...
    constructor(page) {
        ...
        this.emailParserRecord = page.locator('slot[name="output"] div.slds-grid .slds-grow div.slds-grid a[data-navigation="enable"]').last();
        ...
        this.referenceNameValidation = page.locator('[data-field-id="RecordReferenceName_cField"] [slot="output"]').last();

        this.sourceTypeValidation = page.locator('[data-field-id="RecordSource_cField"] span.test-id__field-value').last();


        //------------------BUILD THE FIELD PARSER------------------//

        this.parserRecord = page.locator("[data-label='Field Parser Key']").last();

        this.build = page.locator("[name='ctkemailparser__FieldParser__c.ctkemailparser__Editor']").last();
        ...
        this.testResultProcessed = page.locator(".val_parsed").last();

    }

    async newFieldParser(reference_Name) {

        //Click on the Email Parser Record Name to create another Field Parser
        await this.emailParserRecord.click();
        ...
        for (let j = 0; j < sourceCount; ++j) {
            if (await this.totalnumberofOutputType.locator(".slds-media__body").nth(j).textContent() === this.outputTypeOptionName) {
