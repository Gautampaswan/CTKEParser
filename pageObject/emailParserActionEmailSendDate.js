...
class EmailParserActions {
        ...
        constructor(page) {
           ...
           this.typeValidation = page.locator('[data-field-id="RecordType_cField"] span.test-id__field-value').last();
            ...
            this.fieldMappingNameCount_date = page.locator(".slds-table.slds-table_cell-buffer.slds-table_bordered tbody tr.slds-hint-parent");
            ...
            this.fieldMappingName1 = 'lastName'; //Need to change as per requirement
            ...
            this.sourceDropdown_date = page.locator(".slds-combobox__input-value").nth(47); 
    
            this.sourceValueCount_date = page.locator("[role='option']"); //Need to Change as per requirement.
        ...
        async newEmailParserAction(emailParser_ActionName, object_Type, emailParserAction_Sequence) {
            ...
            // Getting the Text of Email Parser Actions
            const fieldMappingNameText_Date = await this.fieldMappingNameCount_date.locator("td").nth(49).allTextContents();
            console.log(fieldMappingNameText_Date);
    
            //Getting the Count of Email Parser Actions
            const fieldMappingValue_Date = await this.fieldMappingNameCount_date.count();
            console.log(fieldMappingValue_Date);
    
            //Iterate Over Email Parser Actions.
            for (let n = 0; n < fieldMappingValue_Date; ++n) {
    
                const fieldMappingValue_Date = await this.fieldMappingNameCount_date.locator("td").nth(n).textContent();
    
                if (this.fieldMappingName_date.includes(fieldMappingValue_Date)) {
                    await this.fieldMappingNameCount_date.locator("td").nth(n).click();
                    break;
                }
            }
            //---------------------SELECT SOURCE DROPDOWN VALUE----------------------//
            ...
            for (let s = 0; s < sourceOptionValue; ++s) {
                ...
                if (this.fieldMappingName1.includes(sourceOptionValue)) {
            ...
            //Getting the Text of Type Option
            const sourceOptionText_date = await this.sourceValueCount_date.locator(".slds-media__body").allTextContents();
            console.log(sourceOptionText_date);
    
            //Getting the Count of Source Option
            const sourceOptionValue_date = await this.sourceValueCount_date.count();
            console.log(sourceOptionValue_date);
    
            //Iterate Over Source Option.
            for (let t = 0; t < sourceOptionValue_date; ++t) {
                const sourceOptionValue_date = await this.sourceValueCount_date.locator(".slds-media__body").nth(t).textContent();
    
                if (this.fieldMappingName2.includes(sourceOptionValue_date)) {
                    await this.sourceValueCount_date.locator(".slds-media__body").nth(t).click();
                    break;
                }
            }
